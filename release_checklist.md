...
- [ ] `smoke_check.sh` 的关键 API 和文档检查已通过。
...
- [ ] adb 自动播放参数可启动 Demo。
- [ ] adb 自动播放可以指定 `echplay_source`。
- [ ] adb 自动播放可以指定 `echplay_rtsp_transport`。
- [ ] adb 自动截图参数已验证。
- [ ] adb 自动短录制参数已验证。
- [ ] 执行 `cd android && ./tools/collect_diagnostics.sh`。
- [ ] 如有长播测试，已填写 `long_play_test_template.md`。

============================================================
八、后端和跨平台预研
============================================================

- [ ] `PlayerBackend` 接口编译通过。
- [ ] `ECHPlayerBackend` 仍作为默认主后端。
- [ ] `AndroidMediaPlayerBackend` 仅作为 fallback 原型。
- [ ] `v2.9_backend_decision.md` 已更新。
- [ ] `v2.9_cross_platform_research.md` 已更新。
